<?
// $Id: permit_pdf.php,v 1.7 2003/03/23 01:22:30 binary Exp $
define('b1n_PDF_PMT_MAX_CTC', '5');
...
$data = $sql->singleQuery("SELECT * FROM \"view_permit_request\" WHERE leg_id = '" . $leg_id . "'");

if(!is_array($data))
{
    b1n_regGoBackExit('Could not get data from view_permit_request.\nAborting PDF Generation');
}
...
$query = "
    ...
    FROM
        \"view_pmt_ctc\"
    WHERE
        pmt_id = '" . $pmt_id . "' AND (ctc_id IS NULL";

foreach($reg_data['contacts'] as $c)
{
    $query .= " OR ctc_id = '" . b1n_inBd($c) . "'";
}
$query .= ") LIMIT " . b1n_PDF_PMT_MAX_CTC;
...
{
    b1n_regGoBackExit('Could not get contacts for this Permit.\nAborting PDF Generation.');
}
...
$query = "
    ...
    WHERE
        srv_provider = 'P' AND
        srv_id = '" . $reg_data['service'] . "'";

$rs = $sql->singleQuery($query);

if($rs && is_array($rs))
{
    $pdf_data['service'] = $rs['srv_name'];
}
else
{
    b1n_regGoBackExit('Could not get service for this Permit.\nAborting PDF Generation.');
}
...
$pdf->b1n_SetHeader(strtoupper($pdf_data['service'] . "\n" . $data['acf_registry'] . ' ' . $data['acf_model']));
...
if(!empty($data['pmt_phone_country_code']))
{
    $phone_prefix = '+' . $data['pmt_phone_country_code'] . '-';
}

if(!empty($data['pmt_phone_city_code']))
{
    $phone_prefix .= $data['pmt_phone_city_code'] . '-';
}
...
$pdf->Row(array('',  $data['pmt_name']));

if(!empty($data['pmt_phone']))
{
    $pdf->Row(array('Phone',  $phone_prefix . $data['pmt_phone']));
}

if(!empty($data['pmt_fax']))
{
    $pdf->Row(array('Fax',  $phone_prefix . $data['pmt_fax']));
}

if(!empty($data['pmt_mobile']))
{
    $pdf->Row(array('Mobile',  $phone_prefix . $data['pmt_mobile']));
}
...
$pdf->Row(array($pdf_data['service']));
$pdf->Ln(b1n_PDF_LINE_SPACE);
